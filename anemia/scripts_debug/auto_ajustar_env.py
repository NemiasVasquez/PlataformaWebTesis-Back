...
def evaluar_imagen(img, extractor, img_name):
    ...
    # Usar caché para la detección (es lo más lento y no depende de estos params)
    ...
    # Aquí monkey-patch temporal al extractor
...
def optimize():
    ...
    print(f"SI TOMAR: {len(sitomar)}, NO TOMAR: {len(notomar)}")

    # Pre-cargar imágenes para ser rápido
    ...
    print("Iniciando Búsqueda Aleatoria Inteligente (200 iteraciones)...")
    ...
    for i in range(200):
        params = {
            "NITIDEZ_UMBRAL_LAP": random.choice([1.0, 1.5, 2.0, 3.0]),
            "NITIDEZ_UMBRAL_TENENGRAD": random.choice([1.0, 1.5, 2.0, 3.0]),
            "NITIDEZ_UMBRAL_VMLAP_MASCARA": random.choice([5, 10, 15, 20]),
            "CONJUNTIVA_MIN_AREA_PCT": random.choice([0.0001, 0.0005, 0.001]),
            "CONJUNTIVA_MIN_ASPECT_RATIO": random.choice([0.3, 0.5, 0.8]),
            "SEG_MIN_AREA": random.choice([50, 100, 200, 300]),
            "SEG_MIN_ASPECT_PRE": random.choice([1.0, 1.1, 1.2])
        }
        
        actualizar_env(params)
        
        extractor = ConjuntivaExtractor() # Reinstanciar para tomar nuevos params
        ...
        if score > best_score:
            best_score = score
            best_params = params
            print(f"[{i+1}/200] Nuevo Mejor! Score: {score} | SI: {pasados_si}/{len(imgs_si)} | NO: {pasados_no}/{len(imgs_no)}")
            print(f"Params: {params}")

    print("\n--- OPTIMIZACIÓN TERMINADA ---")
    print(f"Aplicando mejores parámetros: {best_params}")
    actualizar_env(best_params)
